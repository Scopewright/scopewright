...
### Features
- **#147 — Agent Maître : boîte de conversation dans admin.html + sync docs** — (1) Nouveau document `docs/MASTER_CONTEXT.md` — synthèse système optimisée AI (~400 lignes, architecture, tables, agents, risques, conventions). (2) Nouvelle Edge Function `ai-master` — Sonnet 4.5, lecture seule, pas de tools, charge `master_context` + `master_claude_md` + `ai_learnings` depuis `app_config`. (3) 6e onglet sidebar admin.html "Agent Maître" — UI conversation complète (messages, input, markdown rendering, tables, code blocks). (4) Sync docs : bouton "Synchroniser les docs" fetch MASTER_CONTEXT.md + CLAUDE.md depuis Netlify et les stocke dans `app_config` (`master_context`, `master_claude_md`). (5) Auto-question initiale : à la première visite du tab, envoie automatiquement une analyse système. (6) Prompt `ai_prompt_master` éditable dans le dropdown Prompts AI. Migration : `sql/master_agent_config.sql`
- **#148+#149 — Agent Maître : bouton global + drawer + tools + sanity checks + changelog** — (1) `shared/master-agent.js` — drawer latéral global (FAB button bas-droite, 420px slide-in) accessible sur 5 pages (calculateur, catalogue, admin, approbation, clients). Chaque page définit `getMasterContext()` pour le contexte live. (2) Edge Function `ai-master` rewritten — 7 tools : 3 read-only auto-executed server-side (`list_learnings`, `read_prompt`, `list_all_prompts`), 4 write tools with client-side Apply/Ignore approval (`update_learning`, `delete_learning`, `update_prompt_section`, `log_prompt_change`). (3) Section-based context loading — `selectRelevantSections()` découpe MASTER_CONTEXT.md par `## N.` headers, score par keyword matching, inclut seulement les sections pertinentes. #147-fix: logs confirmation longueur docs chargés. (4) `shared/sanity-checks.js` — 4 checks déterministes (presRuleKeys, descriptionsNotEmpty, totalNotZero, cascadeOrphans), badge sur FAB. Hook dans `openSubmitModal()`. (5) Prompt changelog : `app_config.prompt_change_log` JSONB array. (6) Admin inline chat replaced by landing page redirecting to shared drawer. Migration : `sql/master_prompt_changelog.sql`
- **Feature: presentation_rule — `exclude` et `detail_bullets` implémentés** — `assembleRoomDescription()` consomme désormais `exclude` (filtre les articles dont le texte client contient un terme listé) et `detail_bullets` (bullets injectés dans la section Détails). Prompts AI mis à jour : `notes` retiré, `detail_bullets` ajouté, seules les clés consommées par le moteur sont documentées (`sections`, `exclude`, `detail_bullets`)
- **#146 — Réordonnancement images par drag & drop** — Les images par pièce dans le calculateur sont réordonnables par drag & drop natif HTML5. Indicateur visuel (ombre navy) sur la position de drop. `reorderGroupImages()` recompacte les `sort_order` (0,1,2...) et sauvegarde en DB. L'ordre se reflète dans renderPreview, quote.html et le PDF
- **Feature: Plans architecturaux — sélection multiple à l'upload** — L'input fichier plans accepte désormais `multiple`. L'utilisateur peut sélectionner plusieurs PDF en une fois. Upload séquentiel avec indicateur de progression globale ("Fichier 2/3..."). Les fichiers non-PDF sont ignorés avec un avertissement
- **Feature: Barèmes — `labor_minutes` additif dans `evaluateLaborModifiers`** — Support d'ajout absolu de minutes MO par département via le champ `labor_minutes` dans les modifiers barèmes. Accepte des nombres fixes (`"Assemblage": 30`) ou des expressions évaluées via `evalFormula` (`"Assemblage": "n_partitions * 12"`). Sémantique : `effective = (catalogue × labor_factor) + labor_minutes_add`. En mode cumulatif, les minutes sont sommées (pas multipliées). Coexiste avec `labor_factor` (multiplicatif) et `material_factor`. Propagé dans : `updateRow` (5 points de calcul prix), popover override (colonne Auto), `computeRentabilityData`, `openRentab`, `debouncedSaveItem` (persistance `labor_auto_modifier`), `openSubmission` (restauration). 10 nouveaux tests (GROUP 29, 292 total)
- **#144 — Admin.html : navigation par volets** — Refonte de la page Administration en layout sidebar + contenu. 5 volets : Présentation (couverture, intro), Catalogue (catégories, groupes, tags, dépenses, taux, nomenclature), Workflow (pipeline, sources, types, CRM), Équipe (permissions, rôles, attribution), Prompts AI (prompts, mémoire). Sidebar sticky 220px avec icônes SVG, active state navy, persistance localStorage. Responsive ≤900px : tabs horizontaux scrollables. Les 19 sections accordion existantes sont préservées intactes (data-panel show/hide CSS pur)

- **#133 suite — `description_format_rules` dynamique depuis `app_config`** — La clé `app_config.description_format_rules` est désormais la source unique des règles de format pour toutes les générations AI de descriptions client. Chargée dynamiquement dans l'edge function `translate` (action `calculateur_description`) et dans `ai-assistant` (prompt estimateur via `buildSystemPrompt`). Fallback hardcodé si absente. Placeholder `{{DESCRIPTION_FORMAT_RULES}}` remplacé à l'exécution. Migration : `sql/description_format_rules.sql`. Section éditable ajoutée dans admin.html (volet Présentation)
- **#145 — Admin Présentation : sections Pourquoi, Étapes, Format description** — Trois nouvelles sections dans le volet Présentation d'admin.html : (1) Page "Pourquoi [Atelier]" — titre, texte riche HTML, upload image. Clés `app_config` : `why_title`, `why_text`, `why_image_url`. (2) Étapes du projet — 8 étapes éditables (titre + description). Clé : `project_steps` (JSONB array). (3) Format description client — textarea pour `description_format_rules`. quote.html consomme les données dynamiques avec fallback sur les valeurs hardcodées. Placeholder `{designer}` dans le texte Pourquoi interpolé avec le nom de l'architecte. Migration : `sql/presentation_sections.sql`

### Bug Fixes
- **Fix: #133 — aiCatalogueExplication() ne remplit pas le JSON** — Quand l'AI retournait les clés `sections`/`exclude` au top-level de l'envelope (structure aplatie) au lieu de les nester dans `.json`, `fixEnvelopeJsonSplit` ne les détectait pas → textarea JSON restait vide. Fix : ajout Case 4 qui extrait les jsonKeys du top-level quand `.json` est absent. Aussi mis à jour les jsonKeys de `'notes'` vers `'detail_bullets'` dans les 3 appels
- **Fix: Modale catalogue — boutons Approuver/Rejeter non réinitialisés** — Après approbation d'un article (bouton passait à "..."), ouvrir un autre article pending gardait "..." au lieu de "Approuver". Fix : `openEditModal()` réinitialise explicitement `textContent` et `disabled` des deux boutons à chaque ouverture
- **Fix: Titre soumission — valeur initiale + champ éditable** — (A) `openProject()` crée la première soumission avec le titre "Soumission 1" au lieu de `project.name` (code projet). (B) Le `<input type="hidden" id="subTitle">` est remplacé par un champ texte visible inline dans le header (`ph-sub-title`), éditable au clic, blur sauvegarde. Verrouillé en lecture seule quand la soumission n'est pas éditable
- **Fix: Assistant contacts — double confirmation (texte + bouton)** — Le prompt contacts-import demandait "Confirmes-tu ?" dans le texte, puis l'UI affichait un bouton Appliquer/Ignorer — double confirmation. Fix : prompt modifié pour que l'AI présente son récapitulatif ET appelle les tools dans la même réponse. Le bouton Appliquer/Ignorer est l'unique mécanisme de confirmation. Règle ajoutée : "NE JAMAIS demander confirmation dans le texte"
- **Fix: Assistant contacts — erreur 400 après collage image** — `buildCtUserContent()` dans clients.html ne créait aucun bloc `{type:"text"}` quand l'utilisateur collait une image sans texte → API Anthropic rejetait avec "non-empty content". Fix : toujours ajouter un bloc text (fallback `[Image jointe]`). Ajout de `sanitizeCtMessages()` en défense profonde : (1) garantit que chaque message user a du contenu non-vide, (2) détecte les tool_use orphelins (sans tool_result) et injecte des résultats synthétiques. Appliqué avant chaque appel à l'edge function contacts-import
- **Fix: #145 — quote.html n'affiche pas les données dynamiques (project_steps, why_*)** — La RLS policy `anon_read_branding` sur `app_config` ne listait que 9 clés (cover_image + intro_*). Les clés ajoutées par #145 (`project_steps`, `why_title`, `why_text`, `why_image_url`) et les traductions EN (`*_en`) étaient bloquées par RLS → quote.html tombait sur le fallback hardcodé. Fix : ajout des 7 clés manquantes à la policy. Migration : `sql/anon_read_presentation.sql`
...
- **Fix: #137 PDF — images coupées par max-height trop restrictif** — `max-height:3.2in` sur `.pv-img-wrap` croppait les images hautes (plans architecturaux). Fix : retrait du `max-height` fixe, le grid CSS distribue la hauteur naturellement. `object-fit:contain` en `@media print` (image complète, lisible) vs `cover` dans le browser (esthétique). `.pv-page-room-media{height:100%}` force le grid à utiliser toute la hauteur du body flex
