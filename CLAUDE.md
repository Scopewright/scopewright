...
- **Step 4a — ID > client_text** (DEC-060) : dans `resolveCascadeTarget`, avant le lookup `client_text` (Step 4b), scanne tous les sous-champs enrichis dans l'ordre `style → materiau → bande_chant → finition → bois_brut`. Le premier `catalogue_item_id` valide dans `CATALOGUE_DATA` + catégorie autorisée (`getAllowedCategoriesForGroup`) → résolution directe par ID. Style en premier = FAB façade prioritaire. Élimine l'ambiguïté multi-articles avec même `client_text`. Fallback : Step 4b (`client_text` matching) si aucun ID valide. **Pas de Tier 0** dans `$default:` — le Tier 0 existe uniquement dans `resolveMatchTarget` pour `$match:`
