...
- Cache choix : `dmChoiceCache[groupId + ':' + typeName]`. **Always-cache** (fix #188) : toute résolution DM (modale, single-match, annulation) cache immédiatement le résultat en session — la checkbox "Mémoriser" n'affecte plus le cache session (elle était redondante). Empêche la modale DM de réapparaître à chaque changement de dimension/tablette/partition
