...
- `DM_ENRICHED_GROUPS` — Caisson: `materiau`+`coupe`+`bande_chant`+`finition`. Façades: `style`+`materiau`+`coupe`+`bande_chant`+`finition`+`bois_brut` (style en premier, texte libre). Panneaux: `materiau`+`coupe`+`bande_chant`+`finition`+`bois_brut`
...
- **Combobox catalogue** : les champs Matériau, Bande de chant, Finition, Bois brut utilisent un combobox avec recherche dans `CATALOGUE_DATA` (`.comp-combobox`). **Matériau type-aware** : filtre par catégorie selon le Type DM sélectionné (`_COMP_MATERIAU_TYPE_FILTER` — Caisson→panneau, Façades→facade/panneau, Panneaux→panneau, Tiroirs→tout, Poignées→poignee). Bande de chant → filtre "bande de chant". Finition → filtre "finition". Bois brut → filtre "bois brut". Dédup par `client_text`, max 30 résultats. Stockage dual : `*_client_text` + `*_catalogue_id`. Fonctions : `_compCbSearch`, `_compCbSelect`, `_compCbClear`, `_COMP_CB_IDS`, `_getCompCbCatFilter`. Les résultats utilisent `onmousedown="event.preventDefault()"` pour empêcher le blur de l'input de voler le focus avant le `onclick`
...
- **Bookmark SVG** : toujours visible (même si `client_text` vide), SVG stroke au repos → filled **persistant** quand composante enregistrée (`.dm-bookmark-btn.saved`). Vérifié au rendu via `buildComposanteName(entry)` + `dm_type` match dans `COMPOSANTES_DATA`. Le label principal de la ligne affiche `buildComposanteName` au lieu de `client_text` quand une composante existe. **Détachement** : `_detachComposanteOnEdit(groupId, idx)` — quand un DM est modifié manuellement (`rdmSelectEnrichedItem`, `saveEnrichedText`, `clearEnrichedField`, `rdmSelectItem`, `updateRoomDmType`), le bookmark repasse en stroke, `composante_id` est supprimé, et le badge groupe est mis à jour via `_updateGroupeBadge`
