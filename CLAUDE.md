...
- `reprocessDefaultCascades(changedGroup, scopeGroupId)` — re-cascade quand un DM change (scopeGroupId obligatoire). Invalide `matchDefaults` (cache `$match:` persisté) ET `dmChoiceCache` (choix DM stale). Exécute sous guard `_cascadeRunning = true` pour empêcher `updateRow → scheduleCascade` de déclencher des cascades parallèles (cause de doublons). Re-trigger les parents avec `$default:` ET `$match:` targets. **Barre de progression** : overlay semi-transparent + barre indeterminate navy 3px + texte "⟳ Recalcul…" pendant l'exécution. Ref-counting (`_cascadeProgressRef`) pour les callers multi-reprocess
