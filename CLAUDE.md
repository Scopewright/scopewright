...
- **Enriched materiau fallback** : dans Step 4 de `resolveCascadeTarget`, si `chosenEntry.client_text` et `chosenEntry.catalogue_item_id` sont vides mais `chosenEntry.materiau.catalogue_item_id` existe → lookup direct dans `CATALOGUE_DATA` → résolution. Couvre les DM enrichis dont `_rebuildDmClientText` n'a pas encore populé `client_text`. **Pas de Tier 0** dans `$default:` — le Tier 0 existe uniquement dans `resolveMatchTarget` pour `$match:` (les deux ne sont PAS symétriques)
