...
7. **Accordion pièces (#175)** : une seule pièce ouverte à la fois. `toggleGroup(groupId)` ferme toutes les autres pièces avant d'ouvrir la cible. Toutes collapsées par défaut à `openSubmission` — le collapse est appliqué **avant** `showCalculator()` pour éviter un flash visuel (fix #187). `aiFocusGroupId` synchronisé automatiquement : pièce ouverte → scope room, toutes collapsées → scope project. `setAiScopeFromScroll` désactivé (no-op) — le focus AI est piloté par l'accordion, plus par le scroll. `addFurnitureGroup` ouvre le nouveau groupe en accordion (sauf pendant le bulk load `skipSave`). `toggleAiFocus` ouvre aussi la pièce ciblée. `onAiFocusChanged()` insère un séparateur visuel (`.ai-scope-separator`) + un message système dans `aiConversation`. `_lastAiFocusGroupId` évite les doublons. Réinitialisé dans `resetAiChat()`
