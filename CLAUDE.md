...
- **`materialCtx`** : contexte cascade local créé par `executeCascade` pour un parent donné. Pré-peuplé depuis le DM correspondant à la **catégorie du parent FAB** (ex: parent "Caisson" → DM "Caisson" → `chosenClientText = "Placage chêne blanc"`). Tous les enfants cascade (`$match:` et `$default:`) héritent automatiquement ce matériau. Modale seulement si le parent a plusieurs DM distincts pour son type. Propagé via `resolveCascadeTarget` → `resolveMatchTarget` → `getDefaultMaterialKeywords` (tier 0 shortcut)
