...
- **Tier 0 enriched dans `$default:`** : dans `resolveCascadeTarget`, après le cache check et avant le choix de DM entry, `getEnrichedDmField` est consulté sur chaque DM entry matchée. Si un sous-champ enrichi (materiau via `ENRICHED_DM_FIELD_MAP`) a `catalogue_item_id` → résolution directe. Si `client_text` seul → lookup `CATALOGUE_DATA` + filtre catégorie. 2+ matchs → peuple `client_text` sur l'entry et laisse le pipeline normal choisir. Symétrique avec le Tier 0 dans `resolveMatchTarget`
