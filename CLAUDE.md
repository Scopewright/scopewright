...
- **Total agrégé collapsé** : quand un parent FAB est collapsé, sa cellule `.cell-total` affiche la somme parent + tous les enfants directs (classe `.aggregate-total`, texte bold navy). `updateCollapsedParentTotal(parentRowId)` lit les totaux via `getRowTotal` (lecture pure). `dataset.individualTotal` stocke le total individuel pour restauration à l'expand. Appelé dans : `toggleCascadeChildren`, `applyCascadeVisibility`, `updateRow` (quand un enfant change et son parent est collapsé), `removeRow`, `toggleShowAllCascade`
