...
- **Quantités enfants** : détection automatique constante vs formule. Si `rule.qty` contient des variables dimensionnelles (`L`, `H`, `P`, `QTY`, `n_tablettes`, `n_partitions`) → résultat = quantité totale, **pas de multiplication** par `rootQty`. Si constante pure (ex: `"2"`) → quantité par unité FAB, multipliée par `rootQty`. Regex : `/\b(L|H|P|QTY|n_tablettes|n_partitions)\b/`
...
- **Règle `skipCascade`** : toute fonction qui appelle `updateRow()` et qui N'EST PAS un changement de dimensions (L/H/P), de n_tablettes/n_partitions, ou d'article catalogue DOIT passer `{ skipCascade: true }`. Fonctions corrigées : `saveOverrides`, `clearOverrides`, `refreshGroupRows` (modificateurs %), AI tools `update_submission_line` et `modify_item` (sans changement dims)
