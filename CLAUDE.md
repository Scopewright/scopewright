...
| Fichier | Rôle | Taille |
|---------|------|--------|
| `calculateur.html` | App principale — projets, pipeline, soumissions, meubles, cascade engine, DM system, AI chatbox, annotations, preview | ~19 531 lignes |
...
| `shared/resolve-materials.js` | ~333 lignes. `resolveMaterialsFromComposante()`, `resolveMaterialsFromDmEntry()`, `fillResolvedMaterials()`, `clearResolvedMaterials()`, `ENRICHED_DM_FIELD_MAP`, `COMPOSANTE_SUBFIELD_MAP` | calculateur |
| `shared/pipeline.js` | ~1 375 lignes. Pipeline commercial : `loadProjects`, `renderTableView`, `renderCardView`, `renderSubmissionsView`, `openProjectDrawer`, `openStatusDropdown`, `openDatePicker`, `filterProjects`, `toggleArchiveProject`, `handleDeleteProject`, et ~25 fonctions associées | calculateur |
...
- **`materialCtx`** : contexte cascade **construit frais pour chaque FAB** depuis son propre DM — jamais hérité du parent (4e paramètre de `executeCascade`). Chaque FAB (racine ou enfant) construit son propre `materialCtx` via DEC-061 sync : `getRelevantComposanteId` trouve la composante pertinente, puis le DM de la catégorie du FAB fournit `chosenClientText`. L'appel récursif passe `null` pour `materialCtx` : `executeCascade(childRowId, depth+1, mergedOverrides, null)` — le FAB enfant construit le sien. **Mis à jour par `$default:` après résolution** : `resolveCascadeTarget` propage le `client_text` de l'article résolu dans `materialCtx.chosenClientText` à chaque sortie (`cache hit`, `single candidate`, `modale technique`). Les `$match:` frères du même FAB scorent ainsi dans le contexte du matériau effectivement résolu par le `$default:` précédent. Sert de **disambiguateur** quand plusieurs DM existent, mais **ne surcharge jamais** un DM unique explicite (ex: DM "Finition" = "Laque polyuréthane" est utilisé tel quel). **Note** : le parent passe toujours les dimensions, qty, `override_children`, tag, et installation au FAB enfant — seul `materialCtx` est construit frais
...
- `DM_ENRICHED_GROUPS` — Caisson: `materiau`+`coupe`+`bande_chant`+`finition`. Façades: `style`+`materiau`+`coupe`+`bande_chant`+`finition`+`bois_brut` (style en premier, combobox catalogue FAB façades). Panneaux: `materiau`+`coupe`+`bande_chant`+`finition`+`bois_brut`. **Lookup normalisé** : `_getEnrichedConfig(type)` — lookup via `normalizeDmType` pour tolérer accent/plural mismatches (`"Facade"` matche `"Façades"`). Utilisé partout au lieu de `DM_ENRICHED_GROUPS[entry.type]` direct
- `DM_ENRICHED_LABELS` : `'materiau': 'Panneau'`
- `ENRICHED_DM_FIELD_MAP` : `'PLACAGE'` → `materiau`, `'PANNEAU'` → `materiau`, `'MATERIAU'`/`'MATÉRIAU'` → `materiau`, `BANDE DE CHANT` → `bande_chant`, `FINITION`/`FINITION BOIS` → `finition`, `BOIS BRUT` → `bois_brut`. **Déplacé de calculateur.html vers `shared/resolve-materials.js`** (source de vérité unique)
- `rdmSearchEnriched` fieldCatMap : `materiau` → `['PLACAGE', 'PANNEAU', 'PANNEAU MÉLAMINE', 'PANNEAU BOIS', 'PANNEAUX', 'MATÉRIAU', 'MATERIAU']`, `style` → `['FACADE', 'FAÇADE']` (FAB only, item_type=fabrication), `bande_chant` → `['BANDE DE CHANT', 'BANDES DE CHANT']`, `finition` → `['FINITION', 'FINITION BOIS', 'FINITIONS']`, `bois_brut` → `['BOIS BRUT']` (jamais `'BOIS'` seul — trop permissif, capture PANNEAU BOIS et FINITION BOIS). **Materiau/Style sans dédup** : les champs `materiau` et `style` affichent chaque article individuellement (`ST-XXXX — Description`, catégorie en secondaire), recherche sur code + description + client_text. Layout flex : gauche = badge code compact (`.dm-ac-code`, 10px, fond subtle) + description, droite = `client_text` en gris discret (`.dm-ac-ct`, `rgba(255,255,255,0.35)`) — omis si vide ou identique à la description. Les autres champs (bande_chant, finition, bois_brut) gardent la dédup par `client_text`. **Backward compat style** : `entry.style` peut être string (legacy), objet `{catalogue_item_id, client_text}` (nouveau), ou string JSON sérialisée (bug stale data). `_dmFieldText(val)` détecte les 3 cas : string plain → retourne directement, string commençant par `{` → `JSON.parse` + `client_text`, objet → `client_text`. Guard `openSubmission` : parse les sous-champs `style/materiau/bande_chant/finition/bois_brut` sérialisés comme strings JSON au chargement. **Affichage enrichi sélectionné** : `_dmEnrichedDisplay(itemId, fallbackText)` — après sélection et au chargement, les champs enrichis affichent `ST-XXXX  Description technique` au lieu du `client_text` seul. Tooltip `title` pour texte tronqué. Appliqué dans `rdmSelectEnrichedItem` (sélection) et `renderEnrichedPanel` (chargement)

**UI** : bouton ▾ sur les lignes DM enrichies → accordion `.rdm-enriched` avec sous-champs. Combobox catalogue pour `materiau`/`bande_chant`/`finition`/`bois_brut` (`rdmSearchEnriched`, `rdmSelectEnrichedItem`). Dropdown `COUPE_TYPES` pour `coupe` (select peuplé depuis `app_config.coupe_types`). **Champ principal readonly** : pour les types enrichis (Caisson/Façades/Panneaux), le champ texte principal est en lecture seule — son contenu est construit automatiquement depuis les sous-champs via `_rebuildDmClientText(groupId, idx)` : `"{panneau}"` pour TOUS les types. **Lookup catalogue frais** : si `entry.materiau.catalogue_item_id` existe, lit le `client_text` depuis `CATALOGUE_DATA` (pas depuis `entry.materiau.client_text` qui peut diverger) — garantit un match exact dans Step 4 de `resolveCascadeTarget`. Style, coupe et finition jamais dans `client_text` — ajoutés uniquement par `buildComposanteName` pour l'affichage (évite duplication). `buildComposanteName` : `[type, style, client_text, coupe, finition]`. `applyComposanteToDm` et `applyGroupeToDm` suivent la même règle : `dm.client_text = materiau_client_text` (pas de coupe/style). Champs vides omis via `filter(Boolean)`. Si composante appliquée → affiche le nom composante. Chaque `rdmSelectEnrichedItem`, `saveEnrichedText` et `clearEnrichedField` déclenche la reconstruction. Champ `coupe` conditionnel : affiché pour placage (éditable) et mélamine (désactivé, "Non applicable"), masqué sinon (`_isDmPlacage` — vérifie `entry.client_text`, `entry.materiau.client_text`, `entry.materiau.catalogue_item_id` catégorie, et `entry.catalogue_item_id` catégorie). Champ `finition` désactivé pour mélamine (`_isDmMelamine`). **Changement vers mélamine** : finition ET coupe retirées automatiquement + toast. Warning non-bloquant si bande de chant incompatible avec matériau principal (fuzzy match `client_text`)
...
- **`materialCtx.composante_id`** : résolu frais par chaque FAB depuis son propre DM via `getRelevantComposanteId` — plus hérité du parent. L'appel récursif passe `null` pour `materialCtx`, chaque FAB enfant construit le sien
...
- **`resolveCascadeTarget`** (`$default:`) : **pas de composante-first** (DEC-068) — `resolveByComposante` retiré pour `$default:`. La résolution passe par FAB-priority (DEC-058, scanne les sous-champs enrichis dans l'ordre `DM_ENRICHED_GROUPS[type].fields`) puis Step 4a (DEC-060, ID direct depuis `style` → `materiau`) puis Step 4b (client_text). `resolveByComposante` mappait `$default:Facades` → `materiau_catalogue_id` au lieu de `style_catalogue_id` — causait la résolution du mauvais article
- **`resolveMatchTarget`** (`$match:`) : composante-first check **préservé** — `resolveByComposante` via `COMPOSANTE_FIELD_MAP` mappe correctement les catégories de dépense vers les sous-champs composante. Cache dans `dmChoiceCache`
...
- **DEC-061 — Chaque FAB trouve sa propre composante** : dans `executeCascade`, `getRelevantComposanteId(catItem, groupId)` est appelé **toujours** (pas seulement si `materialCtx.composante_id` est absent). Chaque FAB enfant trouve sa propre composante basée sur sa propre catégorie via `_getCategoryDmType`, au lieu d'hériter celle du parent. Résout le bug où un FAB Façade héritait la composante Caisson du parent
- **#219b — Per-rule composante_id** : dans la boucle des règles, avant chaque `$default:X` dont le type cible diffère du parent, override `materialCtx.composante_id` avec la composante du DM ciblé. Guard : les deux types doivent être non-vides et différents. Candidat au retrait futur (DEC-061 rend le cross-type lookup moins nécessaire)
- **FAB-priority dans `$default:`** : dans `resolveCascadeTarget`, avant Step 2b, scanne les sous-champs enrichis dans l'ordre défini par `DM_ENRICHED_GROUPS[type].fields` (Façades: style→materiau→..., Caisson/Panneaux: materiau→...). Le premier sous-champ dont `catalogue_item_id` pointe vers un FAB est résolu directement — le FAB cascade ensuite ses propres enfants MAT. **Détection FAB** : `item_type === 'fabrication'` OU (`item_type !== 'materiau'` ET `calculation_rule_ai` non-null). Migration backfill : `sql/backfill_item_type.sql`. Si aucun FAB → pipeline normal (Step 2b → Step 4)
- **Step 4a — ID > client_text** (DEC-060) : dans `resolveCascadeTarget`, avant le lookup `client_text` (Step 4b), scanne uniquement les sous-champs **primaires** `style → materiau`. Les sous-champs spécialisés (`bande_chant`, `finition`, `bois_brut`) sont gérés par `$match:` via `resolveMatchTarget` Tier 0 — les inclure dans Step 4a causait la résolution du mauvais article (ex: bande de chant retournée pour `$default:Caisson` quand le panneau était rejeté par le filtre catégorie). **Pas de filtre catégorie** sur Step 4a : le `catalogue_item_id` direct est un choix explicite de l'estimateur via le DM enrichi. Style en premier = FAB façade prioritaire. Fallback : Step 4b (`client_text` matching) si aucun ID valide. **Pas de Tier 0** dans `$default:` — le Tier 0 existe uniquement dans `resolveMatchTarget` pour `$match:`
...
- **Résolution client_text-only** : si `resolveByComposante` retourne `{ catalogue_item_id: null, client_text: "..." }` (composante sans ID lié), lookup dans `CATALOGUE_DATA` par `client_text` exact + filtre catégorie. 1 match → résolution directe. 2+ → `showTechnicalItemModal`. 0 → fallback normal. Appliqué dans `$match:` composante-first check uniquement (retiré de `$default:` par DEC-068)
...
- **Concept** : chaque FAB stocke un `resolved_materials` JSONB dans `room_items` — mapping avec deux types de clés : `"$default:TypeDM"` pour les articles FAB, UUID de catégorie de dépense pour les articles MAT. La cascade lit ce mapping au lieu de résoudre dynamiquement à chaque exécution. `catalogue_item_id` est la seule source de vérité — les fallbacks `client_text` ont été retirés
- **Module** : `shared/resolve-materials.js` (~333 lignes) — fonctions pures + orchestration. Exporte `resolveMaterialsFromComposante`, `resolveMaterialsFromDmEntry`, `fillResolvedMaterials`, `clearResolvedMaterials`, `ENRICHED_DM_FIELD_MAP`, `COMPOSANTE_SUBFIELD_MAP`
...
- **Phase 2** (implémentée) : intégration dans `executeCascade`. Avant la boucle des règles, `fillResolvedMaterials(parentRowId, groupId)` est appelé pour pré-calculer le mapping. Pour chaque règle `$default:`/`$match:`, le mapping est consulté en premier — si une entrée existe, le `catalogue_item_id` résolu est utilisé directement sans appeler `resolveCascadeTarget`/`resolveMatchTarget`. Legacy préservé comme fallback si le mapping est vide ou ne contient pas l'entrée
- **Phase 3** (implémentée) : `fillResolvedMaterials` appelé avant la boucle des règles cascade. `materialCtx` n'est plus hérité du parent — chaque FAB construit le sien frais. `reprocessDefaultCascades` crash fix : `findCascadeChildren` retourne `{rowId, catalogueId}` objects (pas des éléments DOM)
- **Phase 3b** (implémentée) : `$match:` rules — legacy fallback désactivé. Si `resolved_materials` ne contient pas d'entrée pour la catégorie de dépense → la règle est **skippée** avec toast actionnable "Matériau manquant — re-sélectionnez [category] dans le DM [type]". Pas de fallback vers `resolveMatchTarget`. Élimine le bug racine où `findParentFabRow(groupId)` retournait toujours le Caisson pour les FAB enfants (ex: Façade Slab recevait les matériaux Caisson). `$default:` rules : legacy FAB-priority préservé en fallback (fonctionne correctement car `resolveCascadeTarget` utilise le DM du bon type)
- **Phase 4** (planifiée) : suppression de la résolution legacy `resolveCascadeTarget` pour `$default:` après validation complète
...
| `tests/cascade-helpers.js` | 24 fonctions pures extraites de `calculateur.html` (copies paramétrisées) |
...
- [ ] `materialCtx` construit frais par chaque FAB depuis son propre DM (pas hérité du parent)
- [ ] L'appel récursif passe `null` pour `materialCtx` — chaque FAB enfant construit le sien
