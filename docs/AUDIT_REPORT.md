...
- `authenticatedFetch()` centralisé dans `shared/auth.js` avec refresh proactif et retry sur 401
- `escapeHtml()` / `escapeAttr()` centralisés dans `shared/utils.js`, utilisés de manière consistante
...
- Audit trail AI : `catalogue_change_log` pour les modifications catalogue via AI tools
- Formules cascade évaluées via whitelist de caractères (`evalFormula`)
- CORS restreint aux origines `scopewright.ca`
- Suite de tests cascade : 282 assertions, 28 groupes, 19 fonctions pures, 0 dépendances
- Sanitisation messages AI : défense en profondeur contre `tool_use` orphelins + `content` vide
- Rate limit auto-retry 429/529 avec message utilisateur
...
- **Fichiers monolithiques** — `calculateur.html` (~21 700 lignes) et `catalogue.html` (~8 500 lignes) posent des risques de maintenabilité. Première extraction : `shared/presentation-client.js` (~728 lignes)
...
**[FEATURE-02] Barèmes et modificateurs (`labor_modifiers`)**

Ajustements automatiques de prix basés sur les dimensions. Section séparée dans la modale catalogue (admin only). Structure JSON `{ modifiers: [{ condition, label, labor_factor, material_factor }] }`. Deux modes : **first-match** (défaut, premier modificateur vrai gagne) et **cumulatif** (`"cumulative": true`, tous les vrais sont multipliés). 3 formats de facteur : objet `{dept: mult}`, nombre scalaire, ou clé vide `{"": mult}`. Évalué inline dans `updateRow()`. Fallback : lit `item.labor_modifiers` puis `item.calculation_rule_ai.labor_modifiers`. Colonnes DB : `catalogue_items.labor_modifiers` JSONB, `catalogue_items.labor_modifiers_human` TEXT, `room_items.labor_auto_modifier` JSONB. Action AI `catalogue_labor_modifiers` dans translate. Validation clés au save. Tests : groupes 17-19, 24, 25, 27.

**[FEATURE-03] child_dims — Dimensions enfants calculées**

Quand une règle cascade déclare `child_dims` (ex: `{ L: '(L/n_portes)-0.125', H: 'H-0.25' }`), les dimensions L/H/P de l'enfant sont calculées via `evalFormula` au lieu d'être héritées. `applyChildDims(childRowId, rule, vars)` écrit dans les inputs dim de l'enfant. Recalculé à chaque changement des dims du parent. Tests : groupe 16.

**[FEATURE-04] Variables caisson — n_portes / n_tiroirs**

Inputs `Port.` / `Tir.` dans la zone dims des caissons (même pattern que `n_tablettes`/`n_partitions`). Substitution dans `evalFormula`. Ask completeness : 0 valide, null bloque. AI tools `add_catalogue_item` et `modify_item` supportent les 4 variables caisson. Migration : `sql/caisson_portes_tiroirs.sql`. Tests : groupe 15.

**[FEATURE-05] Filtre catégorie sur $match**

Après résolution d'un `$default:`, le système vérifie si l'article résolu a une relation avec la catégorie du `$match` frère. Check 1 : clés `material_costs` contenant un mot de la catégorie. Check 2 : règles cascade ciblant la même catégorie. Si aucune relation → rejeté silencieusement (pas de toast). Ex: mélamine n'a pas FINITION → `$match:FINITION BOIS` rejeté. Placage a FINITION dans ses cascades → accepté. `_defaultResolvedFresh` flag force la re-résolution des `$match:` quand le DM change. Tests : groupe 20.

**[FEATURE-06] Collapse enfants cascade**

Enfants cascade masqués par défaut (`display: none`). Triangle ▶ sur le parent FAB + badge `(+N)` quand collapsé. Checkbox globale par pièce pour tout montrer. État en mémoire (`_cascadeExpanded`), pas persisté. Calculs, saves, propagation installation fonctionnent sur les enfants masqués.

**[FEATURE-07] Tool update_catalogue_item**

Modifie un article catalogue depuis l'AI. Whitelist stricte de champs, snapshot avant/après, audit trail dans `catalogue_change_log`. Permission `canEditCatalogue` requise. **Auto-exécuté** après confirmation conversationnelle (audit trail DB suffit).

**[FEATURE-08] AI merge protection**

Quand l'AI regénère un JSON `calculation_rule_ai`, les champs `ask`, `override_children`, `child_dims` existants sont préservés (merger avec l'existant).

**[FEATURE-09] Modale catalogue reste ouverte après save**

Toast navy "Sauvegardé ✓" au-dessus du footer (2.5s). L'utilisateur ferme manuellement. Dirty tracking reset après chaque save.

**[FEATURE-10] Cascade suppressed** (`cascade_suppressed`)

Quand l'utilisateur supprime manuellement un enfant cascade, l'ID catalogue est mémorisé (`cascadeSuppressed[parentRowId]` + `room_items.cascade_suppressed` JSONB). L'enfant n'est pas regénéré. Bouton ⊘ pour restaurer. Reset quand l'article parent change. Migration : `sql/cascade_suppressed.sql`.

**[FEATURE-11] Cascade manual edit indicator**

Indicateur bordure indigo `.cascade-manual-edit` quand la quantité ou le prix d'un enfant cascade diverge des valeurs calculées (`dataset.cascadeQty`, `dataset.cascadeUnitPrice`). Bouton ↺ revert vers les valeurs moteur + re-cascade du parent.

**[FEATURE-12] Enfants cascade manuels**

`addRow(groupId, { parentRowId })` crée un enfant `cascade-child` + `cascade-locked` en dehors du moteur automatique. Persisté avec `parent_item_id` + `cascade_locked: true` en DB. Protégé contre update/suppression par le moteur.

**[FEATURE-13] Undo stack**

`_undoStack[]` (max 10 entrées) — snapshot avant suppression ou modification d'overrides. Bouton flottant ↩ (bas gauche, 8s auto-hide). `executeUndo()` restaure le dernier état.

**[FEATURE-14] Parser fractions dims** (`parseFraction`)

Inputs dims `type="text" inputmode="decimal"`. Accepte `3/4`, `1 1/2`, `23 5/8`, `1-3/4`. Conversion auto au blur via `parseFraction`. Tests : groupe 26.

**[FEATURE-15] Dupliquer un article catalogue**

Bouton dans la modale d'édition. Copie toutes les données sauf `is_default` (false) et `status` (pending). Code ST-XXXX auto-généré. Médias et composants non copiés.

**[FEATURE-16] Rentabilité — redesign #132**

4 sections : KPI cards (Vente/Coût direct/Profit tri-state) → bannière AI (marge brute < 35%) → barre répartition → marges + ventilation MO → tableau matériaux → tags. Bouton "Ajuster le prix" (scope group) calcule le prix cible pour 38% et applique via room modifier %. Palette Scopewright desaturée.

**[FEATURE-17] Description AI — panneau proposition #135**

`aiGenerateDescription()` affiche un panneau inline `.ai-desc-proposal` au lieu d'écrire directement. Remplacer tout / Insérer la sélection (contextuel) / Ignorer.
...
**[ARCH-01] CRITIQUE — `calculateur.html` : ~21 700 lignes**

Ce fichier unique contient :
- ~530+ fonctions réparties en 9 namespaces (`App`, `UI`, `Db`, `Cascade`, `Ai`, `Calc`, `Pipeline`, `Workflow`, `Media`)
- ~70 variables globales (accessibles via getters/setters sur les namespaces)
...
- Première extraction réalisée : `shared/presentation-client.js` (~728 lignes, 30 fonctions) — mais reste couplé via 30+ globals
...
**[ARCH-02] IMPORTANT — `catalogue.html` : ~8 500 lignes**

Même problème à moindre échelle. En baisse (était ~9 200).
...
| RN-05 | ~~**Tests automatisés**~~ **IMPLÉMENTÉ** | 282 assertions, 28 groupes dans `tests/cascade-engine.test.js`. Couvre 19 fonctions pures : evalFormula, normalizeDmType, isFormulaQty, computeCascadeQty, mergeOverrideChildren, isRuleOverridden, checkAskCompleteness, inferAskFromDimsConfig, extractMatchKeywords, scoreMatchCandidates, deduplicateDmByClientText, getAllowedCategoriesForGroup, itemHasMaterialCost, findExistingChildForDynamicRule, computeChildDims, evaluateLaborModifiers, checkDefaultItemMatchCategory, parseFraction, computeRentabilityPure | Élevé | Rouler `node tests/cascade-engine.test.js` avant chaque push cascade |
...
| catalogue_change_log | any auth | any auth | — | — | Audit trail AI (immuable) |

---

*Fin du rapport d'audit. Ce document identifie 15 problèmes de sécurité, 29 bugs (dont 23 corrigés), 18 features ajoutées, 13 risques architecturaux et 8 problèmes de performance, avec 27 recommandations priorisées. Dernière mise à jour : 2026-03-08.*
