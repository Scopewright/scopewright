...
- Suite de tests cascade : 189 assertions, 24 groupes, 0 dépendances
...
- **Fichiers monolithiques** — `calculateur.html` (~19 000 lignes) et `catalogue.html` (~9 500 lignes) posent des risques de maintenabilité
...
Ajustements automatiques de prix basés sur les dimensions. Section séparée dans la modale catalogue (admin only). Structure JSON `{ modifiers: [{ condition, label, labor_factor, material_factor }] }`. First-match (premier modificateur vrai gagne). Évalué inline dans `updateRow()` à chaque changement de dimensions. Colonnes DB : `catalogue_items.labor_modifiers` JSONB, `catalogue_items.labor_modifiers_human` TEXT, `room_items.labor_auto_modifier` JSONB. Action AI `catalogue_labor_modifiers` dans translate. Prompt `ai_prompt_labor_modifiers` configurable dans admin. Tests : groupes 17-19.
...
Modifie un article catalogue depuis l'AI. Whitelist stricte de champs, snapshot avant/après, audit trail dans `catalogue_change_log`. Permission `canEditCatalogue` requise. Jamais auto-exécuté — confirmation obligatoire.
...
**[ARCH-01] CRITIQUE — `calculateur.html` : ~19 000 lignes**
...
| RN-05 | ~~**Tests automatisés**~~ **IMPLÉMENTÉ** | 189 assertions, 24 groupes dans `tests/cascade-engine.test.js`. Couvre 17 fonctions pures : evalFormula, normalizeDmType, isFormulaQty, computeCascadeQty, mergeOverrideChildren, isRuleOverridden, checkAskCompleteness, inferAskFromDimsConfig, extractMatchKeywords, scoreMatchCandidates, deduplicateDmByClientText, getAllowedCategoriesForGroup, itemHasMaterialCost, findExistingChildForDynamicRule, computeChildDims, evaluateLaborModifiers, checkDefaultItemMatchCategory | Élevé | Rouler `node tests/cascade-engine.test.js` avant chaque push cascade |
