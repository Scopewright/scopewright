...
- **FAB-priority** (DEC-058) : dans `$default:X`, scanne les sous-champs enrichis dans l'ordre de `DM_ENRICHED_GROUPS[type].fields`. Le premier `catalogue_item_id` pointant vers un FAB (`item_type === 'fabrication'`) est résolu directement — le FAB cascade ses propres enfants. Si aucun FAB → **Tier 0 enriched** (MAT via `getEnrichedDmField`)
