...
**Step 4a — ID > client_text** (DEC-060) : dans `resolveCascadeTarget`, **avant** le lookup `client_text` (Step 4b), si `chosenEntry.materiau.catalogue_item_id` existe et est valide dans `CATALOGUE_DATA` + `getAllowedCategoriesForGroup` → résolution directe par ID unique. Élimine l'ambiguïté quand plusieurs articles partagent le même `client_text`. Propage `materialCtx` + cache normalement. **Fallback** : Step 4b (matching `client_text`) si pas de sous-champ `materiau`, ID invalide, ou catégorie non autorisée. **Pas de Tier 0** dans `$default:` — le Tier 0 existe uniquement dans `resolveMatchTarget` pour `$match:`.
