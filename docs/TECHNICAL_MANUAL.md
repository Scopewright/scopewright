...
**Enriched materiau fallback** (Step 4) : si `chosenEntry.client_text` et `chosenEntry.catalogue_item_id` sont vides mais `chosenEntry.materiau.catalogue_item_id` existe dans `CATALOGUE_DATA` → résolution directe. Couvre les DM enrichis dont `_rebuildDmClientText` n'a pas populé `client_text`. **Pas de Tier 0** dans `$default:` — le Tier 0 (`getEnrichedDmField`) existe uniquement dans `resolveMatchTarget` pour `$match:`. Les deux chemins ne sont pas symétriques : `$default:` résout via `entry.client_text` (pipeline normal), `$match:` résout via expense category mapping.
