...
**Injection coupe composante** : dans la section DM de `assembleRoomDescription`, si une entrée DM a un `composante_id` et que la composante correspondante dans `COMPOSANTES_DATA` a un champ `coupe` non vide, le texte est enrichi : `"{description} coupe {coupe}"` (ex: "Placage chêne blanc coupe faux quartier"). `collectRoomDetail` enrichit aussi `defaultMaterials` avec `coupe` pour que l'AI estimateur et le prompt description aient accès à cette information. Backward compatible : si pas de composante ou coupe vide, aucun changement.
...
- Caisson : panneau, coupe, bande de chant, finition
- Façades / Panneaux : panneau, coupe, bande de chant, finition, bois brut
- Tiroirs / Poignées : panneau seulement
- Groupe : tous les champs (inclut style)

Le label "Panneau" remplace "Matériau" pour les trois types principaux (Caisson, Façades, Panneaux) — reflète le vocabulaire terrain. La clé interne reste `materiau`.
...
**Combobox catalogue** : les champs Panneau, Bande de chant, Finition, Bois brut sont des combobox avec recherche dans `CATALOGUE_DATA`. Filtrage par catégorie catalogue (panneau → toutes, bande de chant → "bande de chant", finition → "finition", bois brut → "bois brut"). Déduplication par `client_text`, max 30 résultats. Stockage dual `*_client_text` + `*_catalogue_id`. Coupe reste en texte libre.
...
**DEC-068 — Pas de resolveByComposante pour `$default:`** : le composante-first check (`resolveByComposante`) a été retiré pour les règles `$default:`. `COMPOSANTE_FIELD_MAP` mappait `$default:Facades` → `materiau_catalogue_id` au lieu de `style_catalogue_id` — causait la résolution du panneau ou de la bande de chant au lieu du FAB façade. FAB-priority + Step 4a scannent les sous-champs dans l'ordre correct (`DM_ENRICHED_GROUPS[type].fields`). `resolveByComposante` reste actif pour `$match:` (via `resolveMatchTarget`).
...
3. **`_rebuildDmClientText`** reconstruit `entry.client_text` depuis les sources nettoyées. **Lookup catalogue frais** : si `entry.materiau.catalogue_item_id` existe, lit le `client_text` directement depuis `CATALOGUE_DATA` (pas depuis `entry.materiau.client_text` qui peut diverger par casse/accents). Garantit un match exact dans Step 4 de `resolveCascadeTarget`.
