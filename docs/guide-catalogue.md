...
4. Pour chaque regle :
   ...
   - **Apres `$default:`** : propage l'ID (`_defaultResolvedId`) et le `client_text` resolu dans `materialCtx` (4 points de sortie : cache hit, **Step 4a ID direct** depuis `materiau.catalogue_item_id`, candidat unique par `client_text`, modale technique). Step 4a (DEC-060) resout par ID unique avant le matching `client_text` — elimine l'ambiguite multi-articles. Marque `materialCtx._updatedBySiblingDefault = true`. Si la resolution est **fraiche** (via `resolveCascadeTarget`, pas via enfant existant), active le flag `_defaultResolvedFresh` pour forcer la re-resolution des `$match:` freres
...
- **AI** : le tool `add_catalogue_item` accepte `parent_item_id` (UUID). `collectRoomDetail` expose `isFabParent: true` + `itemId` sur tout article FAB — preserves dans le slim mapping AI. Si le parent n'est pas identifiable, l'AI doit signaler (pas de fallback racine silencieux)
