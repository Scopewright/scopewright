...
Sous-total (avant rabais) : ${context.subtotalBeforeDiscount || context.grandTotal || 0}$`;
  ...
  if (context.focusRoomDetail) {
    ...
    const itemsStr = (f.items || []).map((it: any, i: number) =>
      `  ${i}. ${it.tag ? '[' + it.tag + '] ' : ''}${it.description} — ${it.qty}×${it.qtyMult && it.qtyMult !== 1 ? ' (QM=' + it.qtyMult + ')' : ''} ${it.unitPrice}$ = ${it.lineTotal}$${it.itemId ? ' [itemId=' + it.itemId + ']' : ''}${it.isFabParent ? ' [FAB parent]' : ''}${it.note ? ' [note: ' + it.note + ']' : ''}${it.presRuleHuman ? '\n     présentation: ' + it.presRuleHuman : ''}`
    ).join("\n");
